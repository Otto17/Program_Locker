...
После установки пароля ввести его ещё раз и нажать кнопку "**Добавить файлы…**", выбрать EXE файл(ы) и добавить их (если используются браузеры, либо программы с проверкой исполняемого файла в фоне, то обязательно в выборе режима **СНЯТЬ** галочку на этих программах, что бы лаунчер НЕ создавался), файлы сразу защитятся, там же можно сразу запустить программу(мы), снять защиту, либо удалить программу(мы) из списка записей.
...
P.S. При добавлении любого браузера в "_Locker Launcher_", обязательно СНИМАТЬ галочку в выборе режима защиты, это связано с механизмами защиты современных браузеров, которые работают в фоне, даже когда браузер закрыт.
